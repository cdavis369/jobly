...
class User {
   ...
   * 
   * Inserts new row into applications table[username, job_id].
   * 
  ...
  static async application(username, jobID) {
    const checkForDuplicate = await db.query(`
                                      ...
                                      [username, jobID]);
    if (checkForDuplicate.rows.length) {
      throw new BadRequestError(`Already applied for job: ${jobID}`);
    }

    const checkForJob = await db.query(`SELECT * FROM jobs WHERE id = $1`, [jobID]);
    if (checkForJob.rows.length === 0) {
      throw new BadRequestError(`No job listed: ${jobID}`);
    }
